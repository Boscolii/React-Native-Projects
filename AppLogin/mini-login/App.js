...
export default function App() {
  ...
  const validar = () => {
    const novosErros = {};
    if (!usuario.trim()) novosErros.usuario = 'Digite um nome de usuario';
    if (!email.includes('@')) novosErros.email = 'E-mail inválido';
    if (senha.length < 6) novosErros.senha = 'Senha deve ter mínimo 6 caracteres';
    if (senha != confirmarSenha) novosErros.senha = 'As senhas devem ser iguais';
    setErros(novosErros);
  ...
  const handleLogin = () => {
    if (validar()) {
      Alert.alert('Login realizado!', `Bem-vindo, ${usuario}! 🎉`);
    }
  };
